...
def load_libsovtoken():
    do_call("sovtoken_init")
...
def _load_cdll() -> CDLL:
    ...
    libsovtoken_prefix_mapping = {"darwin": "lib", "linux": "lib", "linux2": "lib", "win32": ""}
    libsovtoken_suffix_mapping = {"darwin": ".dylib", "linux": ".so", "linux2": ".so", "win32": ".dll"}

    os_name = sys.platform
    logger.debug("_load_cdll: Detected OS name: %s", os_name)

    try:
        libsovtoken_prefix = libsovtoken_prefix_mapping[os_name]
        libsovtoken_suffix = libsovtoken_suffix_mapping[os_name]
    except KeyError:
        logger.error("_load_cdll: OS isn't supported: %s", os_name)
        raise OSError("OS isn't supported: %s", os_name)

    libsovtoken_name = "{0}sovtoken{1}".format(libsovtoken_prefix, libsovtoken_suffix)
    logger.debug("_load_cdll: Resolved libsovtoken name is: %s", libsovtoken_name)

    try:
        res = CDLL(libsovtoken_name)
        logger.debug("_load_cdll: <<< res: %s", res)
        return res
    except OSError as e:
        logger.error("_load_cdll: Can't load libsovtoken: %s", e)
        raise e