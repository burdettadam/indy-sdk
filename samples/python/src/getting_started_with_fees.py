...
async def run():
    ...
    req, payment_method = await payment.build_mint_req(minter_wallet[0], trustee1_did, json.dumps([{"recipient": 'pay:sov:53fVT1frfxyckudNpJP9kb3ndz2LfjwGmLtGE9biK85eB2Cg2' '''minter_wallet[1]''', "amount": 1999999999999999999}]), None)
    ...
    outputs_json = json.dumps([{"recipient" : minter_wallet[1], "amount": minter_wallet[3] - 60000000 - 0},{"recipient": steward_wallet[1]   , "amount": 10000000 },
    ...
    result = await ledger.sign_and_submit_request(pool_handle,minter_wallet[0],trustee1_did,payment_req_json)
    logger.info(result)
    # update payment..
    minter_wallet[3] = (minter_wallet[3] - 60000000 - 70)

    #dids for signing get payment source tx
    (government_payment_did, government_payment_key) = await did.create_and_store_my_did(government_wallet[0], json.dumps({}))
    (faber_payment_did, faber_payment_key) = await did.create_and_store_my_did(faber_wallet[0], json.dumps({}))
    (acme_payment_did, acme_payment_key) = await did.create_and_store_my_did(acme_wallet[0], json.dumps({}))
    (thrift_payment_did, thrift_payment_key) = await did.create_and_store_my_did(thrift_wallet[0], json.dumps({}))
    (alice_payment_did, alice_payment_key) = await did.create_and_store_my_did(alice_wallet[0], json.dumps({}))
    for wallet,__did in [\
            (steward_wallet,steward_did),\
            (minter_wallet,trustee1_did),\
            (government_wallet,government_payment_did),\
            (faber_wallet,faber_payment_did),\
            (acme_wallet,acme_payment_did),\
            (thrift_wallet,thrift_payment_did),\
            (alice_wallet,alice_payment_did),\
                        ]:
        await add_utxo_wallet(pool_handle,wallet,__did)
    sources_info_as_json = await get_payment_source(pool_handle, minter_wallet[0], trustee1_did, minter_wallet[1] )
    logger.info("\"minter\" -> payment sources response: {}, payment type: {}".format(sources_info_as_json,payment_method))

    sources_info_as_json = await get_payment_source(pool_handle, faber_wallet[0], faber_payment_did, faber_wallet[1] )
    logger.info("\"minter\" -> payment sources response: {}, payment type: {}".format(sources_info_as_json,faber_wallet[2]))
    ...
    #logger.info("attrib")
    #attrib_request = await ledger.build_attrib_request(steward_did, steward_did,None,json.dumps({"endpoint":{"ha":"127.0.0.1:5555"}}),None)
    
    #logger.info("sign submit")
    #result = await send_request_with_fees(pool_handle, steward_wallet, 5, attrib_request , steward_did)
    #logger.info(result)
    ...
    #(_, transcript_schema) = await get_schema_with_fees(pool_handle, faber_wallet, 30, faber_did, transcript_schema_id)
    ...
    tails_writer_config = json.dumps({'base_dir': str(Path.home().joinpath(".indy_client").joinpath("tails")), 'uri_pattern': ''})
    ...
    (rev_reg_id, rev_reg_def_json, _) = \
        await anoncreds.issuer_create_and_store_revoc_reg(faber_wallet[0], faber_did, None, rvoc_reg_def_tag,
                                                          faber_transcript_cred_def_id, rvoc_reg_def_config, tails_writer)'''

    logger.info("\"Faber\" -> Send  \"Faber Transcript\" Credential Definition to Ledger")
    await send_cred_def_with_fees(pool_handle, faber_wallet, 10, faber_did, faber_transcript_cred_def_json)
    ##----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    await send_cred_def_with_fees(pool_handle, acme_wallet, 10, acme_did, acme_job_certificate_cred_def_json)
    ...
    logger.info(" Issuer credentail\"\n{} \n{} \n{} \n{} \n{} \n{}\" DID".format(faber_wallet[0], transcript_cred_offer_json,
                                                 transcript_cred_request_json,
                                                 transcript_cred_values, None, None))
    transcript_cred_json, rev_id, rev_reg_delta_json = \
        await anoncreds.issuer_create_credential(faber_wallet[0], transcript_cred_offer_json,
                                                 transcript_cred_request_json, 
                                                 transcript_cred_values, None, None)
    logger.info(rev_id)
    logger.info(rev_reg_delta_json)
    ...
    job_application_proof_request_json = json.dumps({
        ...
        #"non_revoked": {"from": 80, "to": 100}, # for revocation
        #"rev_reg_seq_no": 1 #rev_reg_delta_json["issued"][0]# buggy
    })
    ...
    logger.info(job_application_proof_request_json)
    ...
    logger.info(creds_for_job_application_proof)
    ...
    #logger.info(cred_defs_json)
    #logger.info(revoc_states_json)
